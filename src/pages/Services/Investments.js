...
import { FaUmbrellaBeach, FaChartLine, FaPhoneAlt, FaCheck } from "react-icons/fa";

import { db, auth } from "../../firebase";
import { collection, addDoc, serverTimestamp } from "firebase/firestore";
import FooterBanner from "../../components/FooterBanner"; // <-- added
...
const SmartInvestmentHero = () => {
  ...
  function submitPlan() {
    setSuccessMsg(`Your request for a "${selectedRange}" plan has been received.`);
  ...
  async function submitCall(e) {
    ...
    try {
      ...
      setSuccessMsg(`Thanks ${callForm.name || "there"} — we'll contact you shortly.`);
    ...
    <div className="bg-[#F7F1EC] dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans">
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 md:flex md:items-center md:justify-between relative">
        ...
        >
          ...
          <Link to="/investments2">{/* CTA intentionally left empty as in original */}</Link>
        </motion.div>
      ...
      {/* Removed small info cards (Retirement/Wealth) from the top — both will remain below in the Cards section */}
      ...
      >
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto">
            ...
            >
              Choose a plan range or book a call with our advisors. We provide tailored plans focused on long-term security, growth, and legacy building.
            </motion.p>
          </div>

          {/* Cards (Both Retirement & Wealth kept here) */}
          <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Retirement Planning (kept in the lower cards area) */}
            <motion.article
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">Why Retirement Planning is Important</p>
                </div>
              </div>
              ...
              <ul className="mt-6 space-y-3 text-gray-700 dark:text-gray-300">
                ...
                <li className="flex items-start gap-3">
                  <FaCheck className="mt-1 text-green-600 dark:text-green-400" />
                  <span>Protects you from inflation and increasing living costs</span>
                </li>
                <li className="flex items-start gap-3">
                  <FaCheck className="mt-1 text-green-600 dark:text-green-400" />
                  <span>Provides medical and emergency support in later life</span>
                </li>
                <li className="flex items-start gap-3">
                  <FaCheck className="mt-1 text-green-600 dark:text-green-400" />
                  <span>Reduces dependency on family, loans, or government schemes</span>
                </li>
              </ul>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Wealth Building</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">Why Wealth Building is Important</p>
                </div>
              </div>
              ...
              <ul className="mt-6 space-y-3 text-gray-700 dark:text-gray-300">
                ...
                <li className="flex items-start gap-3">
                  <FaCheck className="mt-1 text-green-600 dark:text-green-400" />
                  <span>Builds long-term wealth for family, legacy, and business</span>
                </li>
              </ul>
        ...
        {planModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div initial={{ y: 20, scale: 0.98 }} animate={{ y: 0, scale: 1 }} transition={{ duration: 0.22 }} className="relative z-10 max-w-lg w-full bg-white dark:bg-gray-800 rounded-xl p-6 shadow-xl border dark:border-gray-700">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Start your plan — {selectedRange}</h4>
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">We'll prepare a personalized plan based on your chosen range and reach out to discuss details & investment instruments.</p>

              <div className="mt-4 flex gap-3">
                <button onClick={submitPlan} className="px-4 py-2 rounded-lg bg-[#8D5A3A] text-white hover:bg-[#7a4b2f]">Confirm & Proceed</button>
                <button onClick={() => setPlanModalOpen(false)} className="px-4 py-2 rounded-lg border">Cancel</button>
              </div>

              {successMsg && <div className="mt-4 text-sm text-green-700 dark:text-green-300">{successMsg}</div>}
            </motion.div>
          </motion.div>
        )}
        ...
        {callModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <div className="absolute inset-0 bg-black/40" onClick={() => setCallModalOpen(false)} />
            <motion.form onSubmit={submitCall} initial={{ y: 20, scale: 0.98 }} animate={{ y: 0, scale: 1 }} transition={{ duration: 0.22 }} className="relative z-10 max-w-lg w-full bg-white dark:bg-gray-800 rounded-xl p-6 shadow-xl border dark:border-gray-700">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Book a Call — {currentPlan.title}</h4>
              <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{currentPlan.description}</p>

              <div className="mt-4 grid grid-cols-1 gap-3">
                <input className="px-3 py-2 rounded-md border bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-sm" placeholder="Name" value={callForm.name} onChange={(e) => setCallForm((p) => ({ ...p, name: e.target.value }))} required />
                <input className="px-3 py-2 rounded-md border bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-sm" placeholder="Phone" value={callForm.phone} onChange={(e) => setCallForm((p) => ({ ...p, phone: e.target.value }))} required />
                <input className="px-3 py-2 rounded-md border bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-sm" placeholder="Email (optional)" value={callForm.email} onChange={(e) => setCallForm((p) => ({ ...p, email: e.target.value }))} />
                <textarea className="px-3 py-2 rounded-md border bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-sm" placeholder="Brief note (your goals / best time to call)" value={callForm.note} onChange={(e) => setCallForm((p) => ({ ...p, note: e.target.value }))} rows={3} />
              </div>

              <div className="mt-4 flex gap-3">
                <button type="submit" className="px-4 py-2 rounded-lg bg-[#8D5A3A] text-white">Request Call</button>
                <button type="button" onClick={() => setCallModalOpen(false)} className="px-4 py-2 rounded-lg border">Cancel</button>
              </div>
